...
// Return if something is in the way between 2 positions
...
const getBlocksBetween = (currentPos, targetPos, moveAxis) => {
  ...
  for (let i = smallerVal + 1; i < biggerVal; i++) blocksBetween.push(axis([i, fixedAxis], [fixedAxis, i]));
...
const isSomethingInTheWayDirectAxis = (currentPos, targetPos, axis) => {
  ...
  const resultArr = blocksBetween.find(position => boardState[returnIdFromPosition(position)].name != 'Empty');
