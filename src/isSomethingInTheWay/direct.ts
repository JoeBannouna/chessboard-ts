...
export const isSomethingInTheWayDirect = (currentPos, targetPos) => {
  ...
  if (curX == tarX) return isSomethingInTheWayDirectAxis(currentPos, targetPos, 'y');
  if (curY == tarY) return isSomethingInTheWayDirectAxis(currentPos, targetPos, 'x');
};

// Get blocks in between 2 positions horizontally or vertically
const getBlocksBetween = (currentPos: SquarePosition, targetPos: SquarePosition, moveAxis: Axis) => {
  const { x: curX, y: curY } = currentPos;
  const { x: tarX, y: tarY } = targetPos;

  // Declare the axis
  const axisIsX = moveAxis == 'x';
  const axis = (x: number | SquarePosition, y: number | SquarePosition): number | SquarePosition => (axisIsX ? x : y);

  // Define all the variables

  // The variable axis value for each the current and target positions
  const curVar = axis(curX, curY) as number;
  const tarVar = axis(tarX, tarY) as number;

  // The fixed axis value
  const fixedAxis = axis(curY, curX) as number;
  ...
  for (let i = smallerVal + 1; i < biggerVal; i++) {
    const square = axis({ x: i, y: fixedAxis }, { x: fixedAxis, y: i }) as SquarePosition;
...
const isSomethingInTheWayDirectAxis = (currentPos: SquarePosition, targetPos: SquarePosition, axis: Axis) => {
